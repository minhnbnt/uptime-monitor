...
Trạng thái này tương ứng y hệt trạng thái trước change gốc `replace-auth-service-with-gotrue` trên `master`. Nhánh `http-only` chỉ khác ở 3 commit ontime gần nhất (streaming + online% calc) — không ảnh hưởng tới thiết kế auth, chỉ làm thay đổi vị trí code cần sửa ở task 6.
...
GoTrue container (`ghcr.io/supabase/gotrue:v2.196.0`) trong `compose.yml` và `compose.infra.yml`, expose cổng riêng `9999`. Client gọi thẳng GoTrue để signup/login/refresh/logout. Traefik chỉ proxy API app (bỏ route `/api/v1/auth`). `GOTRUE_JWT_ISSUER` = URL nội bộ của GoTrue.
...
Thay `XUserIDMiddleware` bằng `AuthMiddleware` verify Bearer token qua `oidc.NewProvider(issuer)` + `IDTokenVerifier` (aud = GoTrue's aud). `GetUserID(ctx)` trả `uuid.UUID` (parse claim `sub`). Provider tự cache JWKS. Mỗi service HTTP cần config `auth.issuer` (vd `http://gotrue:9999`).
...
Thêm DB `gotrue` + user `supabase_auth_admin` + schema `auth` trong `config/init.sql` (theo `init_odic_example.sql` của oidc-example). `GOTRUE_MAILER_AUTOCONFIRM=true` (không cần confirm email khi dev).

**D6 — Bỏ username/name.**
Register/login chỉ email + password. user_metadata không bắt buộc. notification-service `domain.User` rút gọn còn `{ID uuid.UUID, Email string}`; bỏ `username`/`name` ở mọi nơi liên quan user.

**D7 — Sinh key tại build-time/dev-time.**
Tool keygen (Go) đặt trong repo (vd `tools/keygen` hoặc copy từ oidc-example), sinh `.env.gotrue-es256` mẫu.
...
- **ontime refactor**: 3 commit gần nhất đổi cách ontime-service truyền user id (streaming). → Task 6 target code ontime hiện tại, không theo danh sách file cũ của change gốc.

## Migration Plan

1. Proto đổi `user_id` → string, regenerate → các service chưa sửa vẫn compile (vì gọi qua interface).
2. `common/authclient` OIDC middleware + `GetUserID() uuid.UUID`.
...
5. Xoá `auth-service/`, forward-auth, `X-User-ID`, route auth trong api spec, CI matrix, hurl tests.
