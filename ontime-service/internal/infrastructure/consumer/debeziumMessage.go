...
func unmarshalDebeziumMessage(msg redis.XMessage) (debeziumMessage, error) {
	...
	if !ok {
		return debeziumMessage{}, errors.New("missing values field")
	}

	rawStr, ok := raw.(string)
	if !ok {
		return debeziumMessage{}, errors.New("stream message value not string")
	}

	event := debeziumMessage{}
	if err := json.Unmarshal([]byte(rawStr), &event); err != nil {
		return debeziumMessage{}, fmt.Errorf("stream message invalid json: %w", err)
	}
